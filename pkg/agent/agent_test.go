...
func (m *mockLLM) Stream(
	...
	if response.Text != "" {
		if err := emit(llm.Event{Type: llm.EventTextDelta, Text: response.Text}); err != nil {
			return response, err
		}
	}
...
func newTestAgent(t *testing.T, model llm.LLM, tools ...AgentTool) *Agent {
	...
	if err := router.Register(model); err != nil {
		t.Fatal(err)
	}
	a := NewAgent(router)
...
func TestRegisterToolValidation(t *testing.T) {
	...
	if err := a.RegisterTool(&mockTool{}); err == nil {
...
func TestRun_MultiTurnToolLoopPassesSchemaAndUsage(t *testing.T) {
	model := &mockLLM{responses: []llm.GenerateResponse{
		{
			ToolCalls: []llm.ToolCall{
				{ID: "call-1", Name: "echo", Input: json.RawMessage(`{"value":"hello"}`)},
			},
	...
	tool := &mockTool{name: "echo", execute: func(input json.RawMessage) (json.RawMessage, error) {
		gotInput = slices.Clone(input)
		return json.RawMessage(`{"echo":"hello"}`), nil
	}}
	...
	if result.Output != "done" || result.Turns != 2 ||
		result.Usage != (llm.Usage{InputTokens: 25, OutputTokens: 5}) {
		t.Fatalf("unexpected result: %+v", result)
	}
	if string(gotInput) != `{"value":"hello"}` {
		t.Fatalf("input = %s", gotInput)
	}
	if len(model.requests[0].Tools) != 1 || !json.Valid(model.requests[0].Tools[0].InputSchema) {
		t.Fatalf("tool schema not passed: %+v", model.requests[0].Tools)
	}
	last := model.requests[1].Messages[len(model.requests[1].Messages)-1]
	if last.Role != llm.RoleTool || last.ToolCallID != "call-1" {
		t.Fatalf("tool result not correlated: %+v", last)
	}
}
...
func TestRun_TerminalToolReturnsStructuredOutput(t *testing.T) {
				...
				ToolCalls: []llm.ToolCall{
					{
						...
						Input: json.RawMessage(`{"finding":"x"}`),
					},
				},
			},
		},
	...
	tool := &mockTool{
		name:     "report_observation",
		terminal: true,
		execute:  func(input json.RawMessage) (json.RawMessage, error) { return input, nil },
	}
	result, err := newTestAgent(
		t,
		model,
		tool,
	).Run(context.Background(), RunRequest{Provider: "mock", Messages: []llm.Message{{Role: llm.RoleUser, Content: "inspect"}}})
	if err != nil {
		t.Fatal(err)
	}
	if result.StopReason != StopTerminalTool || result.TerminalTool != "report_observation" ||
		string(result.TerminalOutput) != `{"finding":"x"}` {
		t.Fatalf("unexpected result: %+v", result)
	}
}
...
func TestRun_BudgetWrapUp(t *testing.T) {
	model := &mockLLM{responses: []llm.GenerateResponse{
		{
			...
			Usage:     llm.Usage{InputTokens: 70, OutputTokens: 10},
		},
	...
	result, err := newTestAgent(
		...
		model,
		&mockTool{name: "echo"},
	).Run(context.Background(), RunRequest{Provider: "mock", Messages: []llm.Message{{Role: llm.RoleUser, Content: "work"}}, Budget: Budget{MaxTotalTokens: 100, WrapUpReserve: 20}})
	if err != nil {
		t.Fatal(err)
	}
	if result.StopReason != StopTokenBudget || result.Output != "partial summary" ||
		len(model.requests[1].Tools) != 0 {
		t.Fatalf("unexpected wrap-up: %+v", result)
	}
}
...
func TestRun_StreamingAndMiddleware(t *testing.T) {
	...
	a.UseEvents(func(next llm.EventHandler) llm.EventHandler {
		return func(event llm.Event) error {
			if event.Type == llm.EventTextDelta {
				event.Text = "tokenized"
			}
			return next(event)
		}
	})
	...
	result, err := a.Run(
		...
		RunRequest{
			...
			Stream:    true,
			Callbacks: Callbacks{OnText: func(delta string) error { text += delta; return nil }},
		},
	)
	if err != nil {
		t.Fatal(err)
	}
	if !model.stream || text != "tokenized" || result.Output != "secret" {
		t.Fatalf("stream=%v text=%q result=%+v", model.stream, text, result)
	}
}
...
func TestRun_ToolMiddleware(t *testing.T) {
	...
	called := false
	a.UseTool(func(next ToolExecutor) ToolExecutor {
		return func(ctx context.Context, call llm.ToolCall) (json.RawMessage, error) {
			called = true
	...
	_, err := a.Run(
		context.Background(),
		RunRequest{Provider: "mock", Messages: []llm.Message{{Role: llm.RoleUser, Content: "x"}}},
	)
	if err != nil || !called {
...
func TestRun_TurnLimit(t *testing.T) {
	...
	_, err := newTestAgent(
		...
		model,
		&mockTool{name: "echo"},
	).Run(context.Background(), RunRequest{Provider: "mock", Messages: []llm.Message{{Role: llm.RoleUser, Content: "x"}}, MaxTurns: 1})
