...
func (m *mockTool) Execute(_ context.Context, input json.RawMessage) (json.RawMessage, error) {
	...
	return append(json.RawMessage("echo:"), input...), nil
}
...
func TestLLMNode(t *testing.T) {
	...
	node := LLMNode(
		...
		func(v string) testUpdate { return testUpdate{v} },
	)
	update, err := node.Execute(context.Background(), testState{"hello"})
	if err != nil || update.Response != "world" {
...
func TestLLMNodeErrors(t *testing.T) {
	...
	node := LLMNode(
		routerWith(t, &mockLLM{name: "mock", err: boom}),
		"mock",
		func(testState) string { return "x" },
		func(string) testUpdate { return testUpdate{} },
	)
	if _, err := node.Execute(context.Background(), testState{}); !errors.Is(err, boom) {
		t.Fatalf("got %v", err)
	}
	missing := LLMNode(
		llm.NewRouter(),
	...
	if _, err := missing.Execute(context.Background(), testState{}); !errors.As(err, &target) {
		t.Fatalf("got %v", err)
	}
}
...
func TestToolNode(t *testing.T) {
	node := ToolNode(
		&mockTool{},
		func(s testState) json.RawMessage { return json.RawMessage(s.Prompt) },
		func(v string) testUpdate { return testUpdate{v} },
	)
	update, err := node.Execute(context.Background(), testState{"hi"})
	if err != nil || update.Response != "echo:hi" {
		t.Fatalf("update=%+v err=%v", update, err)
	}
}
...
func TestToolNodeError(t *testing.T) {
	...
	node := ToolNode(
		&mockTool{err: boom},
		func(testState) json.RawMessage { return nil },
		func(string) testUpdate { return testUpdate{} },
	)
	if _, err := node.Execute(context.Background(), testState{}); !errors.Is(err, boom) {
		t.Fatalf("got %v", err)
...
func TestAgentNode(t *testing.T) {
	...
	if err != nil || update.Response != "answer" {
		t.Fatalf("update=%+v err=%v", update, err)
	}
}
